...
def parseexpression(expr):
    ...
    expr = re.sub(r'([A-Za-z0-9_]+)\s*>>\s*([A-Za-z0-9_]+)', r'((not \1) or (\2))', expr) # generated by AI
    
    # A <-> B = (A and B) or ((not A) and (not B))
    # make sure to try the derivation yourself! it's very cool
    expr = re.sub(r'IAOI', '==', expr)
    expr = re.sub(r'([A-Za-z0-9_]+)\s*==\s*([A-Za-z0-9_]+)', r'((\1 and \2) or (not \1 and not \2))', expr) # generated by AI
